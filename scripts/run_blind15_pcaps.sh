...
OUT_DIR="capture/pcaps/blind15"
ROUND="15"

mkdir -p "$OUT_DIR"

echo
echo "========== BLIND15 — NOVO TESTE CEGO REAL — VERSÃO SEGURA =========="
...
echo
echo "========== CRIANDO LABELS DO BLIND15 =========="
cat > "$OUT_DIR/pcap_labels_blind15.csv" <<LABELS
filename,label,attack_type
benign_15.pcap,0,benign
scan_15.pcap,1,scan
bruteforce_15.pcap,1,bruteforce
c2_15.pcap,1,c2_beacon
flood_15.pcap,1,dos_flood
mqtt_abuse_15.pcap,1,mqtt_abuse
slow_dos_15.pcap,1,slow_dos
LABELS

cat "$OUT_DIR/pcap_labels_blind15.csv"
...
run_attack "scan" 45s \
  python -u rodada5_generators.py scan --rounds 10 --delay 0.055

run_attack "bruteforce" 70s \
  python -u rodada5_generators.py bruteforce --count 105 --delay 0.045

run_attack "c2" 90s \
  python -u rodada5_generators.py c2 --count 90 --interval 0.55 --jitter 0.35 --payload-size 180

run_attack "flood" 45s \
  python -u rodada5_generators.py flood --count 580 --delay 0.007 --payload-size 300

run_attack "slow_dos" 90s \
  python -u rodada5_generators.py slow_dos --connections 80 --chunks 10 --chunk-delay 0.75 --hold-time 16 --payload-size 75 --conn-delay 0.022

run_attack "mqtt_abuse" 75s \
  python -u rodada5_generators.py mqtt_abuse --connections 80 --messages-per-conn 9 --topics 12 --payload-size 260 --msg-delay 0.014 --conn-delay 0.018
...
echo
echo "========== GERANDO SHA256 =========="
sha256sum "$OUT_DIR"/*.pcap > "$OUT_DIR/SHA256SUMS_blind15.txt"
cat "$OUT_DIR/SHA256SUMS_blind15.txt"
